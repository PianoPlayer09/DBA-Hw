    ...
    while response != '0':
        ...
        if response == "1":  # Add an item
            ...
            if category_name in CATEGORY_LIST:
                ...
                for item_name, item_category in inventory_categories.items():
...
def display_all_inventory(inventory_counts, inventory_costs, inventory_categories):
    ...
    if not inventory_counts and not inventory_costs and not inventory_categories:
        print("== Empty ==")
    else:
        print(f'{"Item name":<15} {"Count":>5} {"Unit Cost":>9} {"Category":<8}')
        print(f'{"---------":<15} {"------":>5} {"-------":>9} {"--------":<8}')

        for item_name in inventory_counts:
            print(f'{item_name:<15} {inventory_counts[item_name]:>5} {inventory_costs[item_name]:>9.2f} {inventory_categories[item_name]:<8}')
            #This was REAlly helpful; https://www.geeksforgeeks.org/python/string-alignment-in-python-f-string/
...
def read_inventory_file():
    ...
    try:
      with open('inventory.dat', 'rb') as f:
        ...
        pass #basically will do nothign and return empty
